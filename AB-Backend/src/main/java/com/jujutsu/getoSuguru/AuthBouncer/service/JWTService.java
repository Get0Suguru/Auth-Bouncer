...
//#Public methods----------------------Jwts class will help with all (parsing, validation, building etc)----------------------------------
    ...
    public boolean isTokenValid(String token){ return (userRepo.existsByUsername(extractUsername(token)) && !isTokenExpired(token));}
...
// ------------------------------------------------------------------------------------------------------------------------
    ...
    private String buildToken(User user, Long expiration){
        ...
        return Jwts.builder()                                     // to make jwt token  (scratch build)
                .subject(user.getUsername())                      // subject is a predefined payload in jwt
